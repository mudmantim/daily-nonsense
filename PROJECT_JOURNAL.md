...
**Verification addendum (same day):** Tim reported the Git connection is done; this commit is the live test — pushing it to `master` and then checking `list_deployments` for a new deployment that Claude did *not* trigger via `deploy_to_vercel`. Result recorded in the next journal entry.
