...
**Second verification attempt (same day) — this commit is the test.** Tim confirmed he checked the dashboard and the connection now shows connected. This commit (and the journal edit producing it) is round two of the same test: push to `master`, then check `list_deployments` for a new deployment neither triggered nor confirmed by a `deploy_to_vercel` call. Result recorded immediately below once known.
