...
| 10 | Deployment / going live | **App is live (Session 8). Git-linking specifically requires Tim (Session 9 finding).** Connecting a GitHub repo to a Vercel project routes through authorizing Vercel's GitHub App for that repo — an access grant, not a config value. No Vercel MCP tool exposes this, and no local CLI/token exists to do it non-interactively either. This is the "authentication or security requires it" exception, not a gap Claude can close solo | **No** — genuinely requires a human authorization step, by Vercel's own design | Low in the interim (snapshot deploys work fine, just require a manual redeploy instead of auto-deploying on push) | **Action needed from Tim:** vercel.com → `daily-nonsense` project → Settings → Git → Connect Git Repository → select `mudmantim/daily-nonsense` → confirm production branch `master`. Under a minute. Once done, Claude will verify a push triggers a real deployment and record it in `PROJECT_JOURNAL.md`. Also move `NEXT_PUBLIC_SITE_URL` from its current code fallback to a real project env var once dashboard access makes that routine | **Blocked on one specific Tim action — everything else about deployment is done** |
