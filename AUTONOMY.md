...
| 10 | Deployment / going live | **Fully done (Session 9).** App live since Session 8; git-linking required Tim's one-time GitHub App authorization (an access grant Claude has no tool to perform), completed and verified with a real push-triggered deployment (`dpl_5rhQd3RN39WVzdPE31usw22okjVK`, `source: "git"`) | N/A — done | N/A | Remaining nice-to-have: move `NEXT_PUBLIC_SITE_URL` from its code fallback to a real project env var now that dashboard access is routine | Low — the one thing actually worth doing next is the env var, whenever convenient |
